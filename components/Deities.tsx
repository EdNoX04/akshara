...
/* ── ŚIVA · the triśūla head ─────────────────────────────────────
   Three bladed prongs on a collared socket, cut as one solid form
   with the fullers and bands incised out of it. Exported bare as
   well, so the Śiva section can use it at full size. */
/** The triśūla head on its own, for use as a page element. */
export function TrishulaMark({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 240 320" fill="none" aria-hidden>
      <g fill={S} fillOpacity=".72">
